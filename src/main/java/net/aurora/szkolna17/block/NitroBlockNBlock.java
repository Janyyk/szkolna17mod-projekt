...
public class NitroBlockNBlock extends Szkolna17ModElements.ModElement {
	@ObjectHolder("szkolna17:nitro_block_n")
	public static final Block block = null;
	public NitroBlockNBlock(Szkolna17ModElements instance) {
		super(instance, 33);
	}
	...
	public static class CustomBlock extends Block {
		public CustomBlock() {
			super(Block.Properties.create(Material.ROCK).sound(SoundType.STONE).hardnessAndResistance(2f, 10f).lightValue(8));
			setRegistryName("nitro_block_n");
		}
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("nitro_block_n", "nitro_block_n", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == NitroBlockBlock.block.getDefaultState().getBlock())
					blockCriteria = true;
				if (blockAt.getBlock() == Blocks.LIME_TERRACOTTA.getDefaultState().getBlock())
					blockCriteria = true;
